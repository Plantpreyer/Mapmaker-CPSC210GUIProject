...
public class CustomMap implements Writable {
    ...
    // break;
    // case "right":
    // viewCenter.setXpos(Math.max(MAP_MAX_SIZE, viewCenter.getXpos() +
    // SHIFT_SIZE));
    // break;
    // case "up":
    ...
    // break;
    // case "down":
    // viewCenter.setXpos(Math.min(MAP_MAX_SIZE, viewCenter.getXpos() +
    // SHIFT_SIZE));
    // break;
    // default:
    // break;
    ...
    // REQUIRES: name in lowercase
    // EFFECTS: returns first index with name in routes
    public int findRouteIndex(String name) throws InvalidInputException {
        for (int i = 0; i < routes.size(); i++) {
            Route b = routes.get(i);
            if (name.equals(b.getName().toLowerCase())) {
                return i;
    ...
    // REQUIRES: selectRoute in routes
    // MODIFIES: this
    // EFFECTS: replaces route with a new one
    public void editRoute(Route selectRoute, Route newRoute) throws InvalidInputException {
        int index = findRouteIndex(selectRoute.getName().toLowerCase());
